...
class Users extends Controller with MongoController {
  ...
  def histchanges: JSONCollection = db.collection[JSONCollection]("histchanges")

  // ------------------------------------------ //
  // Using case classes + Json Writes and products/newReads //
  // ------------------------------------------ //
     ...
     */
      request.body.validate[User].map {
        ...
        // `user` is an instance of the case class `models.User`
          ...
          val auditRecord =  Json.obj(
            "timestamp" ->  Calendar.getInstance().getTime().toString,
            "idNumber" -> user.idNumber,
            "type" -> "create"
          )

          histchanges.insert(auditRecord).map {
            lastError =>
              logger.debug(s"Successfully created with LastError: $lastError")
              Created(s"User Created")
          }
  ...
  def updateUser(firstName: String, lastName: String) = Action.async(parse.json) {
    request =>
      ...
      }.getOrElse(Future.successful(BadRequest("invalid json during updateUser")))
  }

  def deleteUser(idNumber : Int)  = Action.async(parse.json) {
    request =>
      request.body.validate[User].map {
        user =>
          //find the user to delete by id number
          val idNumberSelector = Json.obj("idNumber" -> idNumber)
          collection.remove(idNumberSelector).map {
            lastError =>
